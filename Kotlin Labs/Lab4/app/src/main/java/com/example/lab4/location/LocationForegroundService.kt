...
class LocationForegroundService : Service() {
        ...
        when (intent?.action) {
            ...
            LocationContract.ACTION_START, null -> {
                ...
                try {
                    ...
                    Log.e(tag, "Failed to start foreground/location", e)
                    stopSelf()
                    return START_NOT_STICKY
                }
                return START_STICKY
    ...
    private fun startLocationUpdates() {
        ...
        fusedClient.lastLocation
            .addOnSuccessListener { location ->
                if (location != null) {
                    Log.i(tag, "lastLocation: ${location.latitude}, ${location.longitude} ±${location.accuracy}m")
        ...
        val cb = object : LocationCallback() {
            override fun onLocationResult(result: LocationResult) {
                ...
                Log.i(tag, "update: ${location.latitude}, ${location.longitude} ±${location.accuracy}m")
                broadcastLocation(location.latitude, location.longitude, location.accuracy, location.time)
            }
        }
        callback = cb
    ...
    private fun broadcastLocation(lat: Double, lon: Double, acc: Float, timeMs: Long) {
        val intent = Intent(LocationContract.ACTION_LOCATION).apply {
            // Make broadcast explicit to this app to avoid implicit broadcast delivery issues.
